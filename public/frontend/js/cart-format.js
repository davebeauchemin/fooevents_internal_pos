(function () {
  var cartCfg = typeof window.fiposCartFormat === 'object' && window.fiposCartFormat ? window.fiposCartFormat : {};

  function cartLocale() {
    if (cartCfg.locale && String(cartCfg.locale).trim()) {
      return String(cartCfg.locale).trim();
    }
  ...
  // Human-readable date line for cart (locale from WP / <html lang>).
  function kbmFormatDate(str) {
    var date = new Date(str);
    if (isNaN(date)) return str;
    try {
      return date.toLocaleDateString(cartLocale(), { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
    } catch (e) {
