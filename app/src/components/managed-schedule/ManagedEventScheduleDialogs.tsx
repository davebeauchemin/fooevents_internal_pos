...
export function ManagedEventScheduleDialogs( {
	...
	}, [ capacityOpen, setBulkReduceConfirmOpen ] );
	...
	return (
		<>
					...
					<div className="scrollbar-thin flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-6 pb-10 pt-6">
						<DialogHeader className="shrink-0">
							...
							<DialogDescription className="text-left leading-relaxed">
								Use the same blocks, weekdays, open/close times, and session length as Add missing sessions to{ ' ' }
								<strong className="text-foreground font-medium">delete entire sessions</strong> that match the
								pattern. This dialog only removes calendar cells — use{ ' ' }
								<strong className="text-foreground font-medium">Adjust capacity</strong> above to change numeric
								capacity without deleting sessions. Patterns can include days before site today (unlike Fill empty).
							</DialogDescription>
						</DialogHeader>
			...
			<Dialog open={ capacityOpen } onOpenChange={ onCapacityOpenChange }>
				<DialogContent className="flex max-h-[90vh] max-w-lg flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
					<div className="scrollbar-thin flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-6 pb-10 pt-6">
						<DialogHeader className="shrink-0">
							<DialogTitle>Adjust capacity</DialogTitle>
							<DialogDescription className="text-left leading-relaxed">
								Use the same blocks, weekdays, open/close times, and session length as Add missing sessions. Sessions
								stay on the calendar — you change numeric capacity only (unlimited slots are skipped). Patterns
								can include days before site today (unlike Fill empty).
							</DialogDescription>
						</DialogHeader>
						...
						<section
							className="border-border bg-muted/25 space-y-4 rounded-lg border p-4 sm:p-5"
							aria-labelledby="dlg-capacity-bulk"
						>
							<div className="space-y-2">
								<h2 id="dlg-capacity-bulk" className="text-lg font-semibold tracking-tight">
									Schedule window &amp; preview
								</h2>
								<p className="text-muted-foreground text-sm leading-relaxed">
									The range is the union of each block&apos;s start/end dates (Y-m-d), clipped per block
							...
							<Card className="border-border/80 bg-background/80">
								...
								<CardContent className="space-y-2 text-sm">
									<p className="text-muted-foreground text-xs">
										Block names only
										<strong className="text-foreground font-medium"> group preview rows </strong>.
										Matching uses stepped start times (same as Remove time block).
									</p>
									<p>
										<span className="text-muted-foreground">
											Candidate slot–date positions in merge window:
													...
													&& typeof t.targetTotalCapacity === 'number' ? (
														<>
															...
															{ t.addSpots >= 1 ? (
																<>
																	+
																	<span className="tabular-nums text-foreground">{ t.addSpots }</span>
																</>
															) : (
																<>
																	−
